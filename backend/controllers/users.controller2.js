...
try {
    // ต้องมี auth middleware มาก่อน เพื่อให้ได้ req.user
    const { id, role } = req.user || {};
    console.log("list2 called by:", req.user);
    if (!id || !role) {
    ...
    // role อื่น ๆ: คืนเฉพาะผู้ใช้คนปัจจุบัน
    ...
    return res.json({
      success: true,
      items: me ? [me] : [],  // <- array เสมอ เพื่อให้รูปแบบคงที่
      total: me ? 1 : 0,
      page: 1,
      itemsPerPage: me ? 1 : 0
    });
