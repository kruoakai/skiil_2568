...
('ACD','ฝ่ายวิชาการ'),
('STD','ฝ่ายกิจการนักเรียน'),
('HR','ฝ่ายบุคลากร'),
('FIN','ฝ่ายงบประมาณและพัสดุ'),
('PLA','ฝ่ายแผนงานและประกันคุณภาพ'),
('GUD','งานแนะแนว');
...
-- Users
INSERT INTO users (email, password, name_th, role, department_id, org_group_id) VALUES
('admin@example.ac.th', '$2a$10$exampleexampleexampleexampleexampleexamp', 'ผู้ดูแลระบบ', 'admin',
 (SELECT id FROM departments WHERE code='IT'), (SELECT id FROM org_groups WHERE code='ACD')),
('eva.me@example.ac.th', '$2a$10$exampleexampleexampleexampleexampleexamp', 'ครูประเมิน เครื่องกล', 'evaluator',
 (SELECT id FROM departments WHERE code='ME'), (SELECT id FROM org_groups WHERE code='ACD')),
('eva.it@example.ac.th', '$2a$10$exampleexampleexampleexampleexampleexamp', 'ครูประเมิน ไอที', 'evaluator',
 (SELECT id FROM departments WHERE code='IT'), (SELECT id FROM org_groups WHERE code='ACD')),
('t.it01@example.ac.th', '$2a$10$exampleexampleexampleexampleexampleexamp', 'ครูไอที 01', 'evaluatee',
 (SELECT id FROM departments WHERE code='IT'), (SELECT id FROM org_groups WHERE code='ACD')),
('t.me01@example.ac.th', '$2a$10$exampleexampleexampleexampleexampleexamp', 'ครูเครื่องกล 01', 'evaluatee',
 (SELECT id FROM departments WHERE code='ME'), (SELECT id FROM org_groups WHERE code='ACD')),
('t.acc01@example.ac.th', '$2a$10$exampleexampleexampleexampleexampleexamp', 'ครูบัญชี 01', 'evaluatee',
...
VALUES ('Y2567','การประเมินครูประจำปี 2567', 2567, '2024-05-01','2025-03-31', 1);
...
-- Indicators (แก้ไวยากรณ์: ตัดเครื่องหมาย ] ออก)
...
VALUES
((SELECT id FROM evaluation_periods WHERE code='Y2567'),
 (SELECT id FROM users WHERE email='eva.it@example.ac.th'),
 (SELECT id FROM users WHERE email='t.it01@example.ac.th'),
 (SELECT id FROM departments WHERE code='IT')),
((SELECT id FROM evaluation_periods WHERE code='Y2567'),
 (SELECT id FROM users WHERE email='eva.me@example.ac.th'),
 (SELECT id FROM users WHERE email='t.me01@example.ac.th'),
 (SELECT id FROM departments WHERE code='ME')),
((SELECT id FROM evaluation_periods WHERE code='Y2567'),
 (SELECT id FROM users WHERE email='eva.it@example.ac.th'),
 (SELECT id FROM users WHERE email='t.acc01@example.ac.th'),
...
JOIN users u_evale ON u_evale.email='t.it01@example.ac.th'
JOIN users u_evalr ON u_evalr.email='eva.it@example.ac.th'
JOIN evaluation_topics t ON t.code='TOP1'
JOIN indicators i ON i.code='T1-MEDIA'
WHERE p.code='Y2567';

-- Two example attachments (fix: ปิดวงเล็บครบ)
INSERT INTO attachments (period_id, evaluatee_id, indicator_id, evidence_type_id, file_name, mime_type, size_bytes, storage_path)
SELECT (SELECT id FROM evaluation_periods WHERE code='Y2567'),
       (SELECT id FROM users WHERE email='t.it01@example.ac.th'),
       (SELECT id FROM indicators WHERE code='T1-PLAN'),
       (SELECT id FROM evidence_types WHERE code='E-PLAN'),
       'lesson_plan_it01_2567.pdf','application/pdf',523000,
       '/var/lib/evaluation/uploads/2567/t.it01/lesson_plan_it01_2567.pdf';
...
SELECT (SELECT id FROM evaluation_periods WHERE code='Y2567'),
       ...
       '/var/lib/evaluation/uploads/2567/t.it01/media_samples_it01.zip';
